...
# GISwd <- c("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/ArcGIS/all original layers WGS84 export")
corinewd <- c("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/data/corine raster 100x100")
cuckoowd <- c("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/data")
cuckoowd2 <- c("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/data/resampled data with uncertainty 100 bootstraps + distance, movement groups, etc")
...
############## DON'T NEED THIS SECTION EVERY TIME ##################
...
for (a in 1:31){
  ...
  setwd("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/output/Europe-Africa autumn maps")
...
for (a in 1:31) {
  ...
  setwd("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/data/original data + distance, movement groups, etc")
  originalfiles <- list.files()
  
  originalcuckoo <- read.csv(originalfiles[a], header=T)
  ...
  # stopovers <- Eurcuckoo[which(Eurcuckoo$mtype=="C" | Eurcuckoo$mtype=="S" | is.na(Eurcuckoo$mtype)),]
  
  # dataset includes capture (breeding) location, stopovers, as well as last known location (if in Europe)
  #stopovers <- stopovers[which(stopovers$mgroup!=1),] # this line removes the capture occasion
  
  autumn <- Eurcuckoo[which(Eurcuckoo$julian >= 131),] # all julian dates after May 10 (removes MOST - but not necessarily all - spring stopovers)
  ...
  newautumn.orig <- autumn.orig
  
  for (i in 1:length(levels(autumn.orig$mgroup))) {
    temp <- newautumn.orig[newautumn.orig$mgroup==levels(autumn.orig$mgroup)[i],]
    temp@data <- droplevels(temp@data)
    if (length(levels(as.factor(temp$tcycle))) < 2) {
      newautumn.orig <- newautumn.orig[which(newautumn.orig$mgroup!=levels(autumn.orig$mgroup)[i]),]
      newautumn.orig@data <- droplevels(newautumn.orig@data)
    } else {
      newautumn.orig <- newautumn.orig
    }
  }
  
  dat.orig <- newautumn.orig
  ...
  dir.create(paste("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/output/Europe maps/", levels(newdat$name), sep=""))
  outputdir <- paste("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/output/Europe maps/", levels(newdat$name), sep="")
  setwd(outputdir)
...
setwd("C:/Users/samf/Documents/Sam's Stuff/R/projects/cuckoos/output/Europe maps/")
