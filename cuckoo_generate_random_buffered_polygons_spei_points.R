...
#### FUNCTION TO GENERATE RANDOM DISTANCE FROM ORIGINAL CENTROID ####
...
repeat { # OUTER REPEAT LOOP - regenerates a new random polygon if buffer limit is reached
  ...
  ### CREATE RANDOMIZED POINTS and POLYGONS FOR PSEUDO-ABSENCES
  
  # for each stopover, generate x,y distance to move all points within a stop (based on centroid of original stopover)
  movepointby <- t(sapply(allstopSPDFs@polygons, chooserandompoint))
  
  # elide all points and polygons to new random location
  split.newdat <- split(newdat, newdat$mgroup)
  split.randomdat <- list()  
  split.allstopSPDFs <- split(allstopSPDFs, allstopSPDFs$mgroup)
  split.randomstopSPDFs <- list()
  
  for (i in 1:length(split.newdat)) {
    split.randomdat[[i]] <- elide(split.newdat[[i]], shift=movepointby[i,])
    split.randomstopSPDFs[[i]] <- elide(split.allstopSPDFs[[i]], shift=movepointby[i,])
  }
  
  randomdat <- do.call(rbind,split.randomdat)
  proj4string(randomdat) <- corine.crs
  colnames(randomdat@coords) <- c("randomlong","randomlat")
  randomstopSPDFs <- do.call(rbind,split.randomstopSPDFs)
  proj4string(randomstopSPDFs) <- corine.crs
  
  ### Test plot location of random points and polygons in relation to original
  # test.pts <- split.randomdat[[1]]
  # test.poly <- split.randomstop[[1]]
  # test.r <- crop(r, extend(extent(test.poly), 10000))
  # 
  # plot(test.r)
  # plot(split.allstopSPDFs[[1]], col="red", add=TRUE)
  # plot(test.poly, col="blue", add=TRUE)
  # plot(split.newdat[[1]], pch=16, col="black", add=TRUE)
  # plot(test.pts, pch=16, col="black", add=TRUE)
  
  # transform from EPSG 3035 to same CRS as climate rasters (EPSG 4326)
  randomdat4326 <- spTransform(randomdat, CRS=epsg4326)
